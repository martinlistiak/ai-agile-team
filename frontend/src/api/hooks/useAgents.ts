...
export function useUpdateAgentSystemPrompt(spaceId: string | null) {
  const queryClient = useQueryClient();
  const { error: showError } = useToast();

  return useMutation({
    mutationFn: async (payload: { id: string; systemPrompt: string }) => {
      const { id, ...body } = payload;
      const { data } = await api.patch(`/agents/${id}/system-prompt`, body);
      return data as Agent;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["agents", spaceId] });
    ...
    onError: () => {
      showError("Failed to update agent instructions");
    },
  });
}
