...
export class AgentRunQuotaService {
  ...
  /**
   * Enforces Starter daily agent run limit for the space owner.
   * Team and Enterprise are unlimited.
   */
  async assertCanStartRunForSpace(spaceId: string): Promise<void> {
    ...
    if (count >= STARTER_DAILY_AGENT_RUNS) {
      throw new ForbiddenException({
        code: API_ERROR_AGENT_RUN_QUOTA_EXCEEDED,
        message: `You've reached your Starter plan limit of ${STARTER_DAILY_AGENT_RUNS} AI agent runs today (UTC). Upgrade to Team for unlimited runs and higher usage.`,
      });
    }
  }
}
